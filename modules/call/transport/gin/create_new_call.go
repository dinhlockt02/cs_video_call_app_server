...
func CreateNewCall(appCtx appcontext.AppContext) gin.HandlerFunc {
	return func(context *gin.Context) {
		...
		createNewCallBiz := callbiz.NewCreateNewCallBiz(appCtx.Notification(), callRepo)
		if err := createNewCallBiz.CreateNewCall(context.Request.Context(), requesterId, friendId, callRoomId); err != nil {
			panic(err)
		}
